...
void Insert(PData x, PNode pl)
{
    ...
    if (len) tmp -> data = MALLOC(len);
    else     tmp -> data = x;  //指针链表
...
{
    u_16 i;
    if (node_num > ListLen(pl)) return NULL;
...
{
    PNode p;
    if (num > ListLen(pl)) return ADT_ERR;
    p = GetNodePtr(pl, num);
    Insert(x, p);
    return ADT_OK;
}
...
{
    PNode p;
    if (num > ListLen(pl) || num == 0) return ADT_ERR;
    p = GetNodePtr(pl, num);
    Delete(pl, p);
    return ADT_OK;
}
